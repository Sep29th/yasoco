...
export const examinationDataToInvoiceTemplateData = (examination: ExaminationDataNeededForInvoiceTemplateType): InvoiceTemplateDataType => {
	let serviceTotal = 0;
	let medicineTotal = 0;
	...
	const medicines = examination.medicines.map((medicine, indx) => {
		const sum = medicine.price * medicine.quantity;
		medicineTotal += sum;
		return {
			...medicine,
			order: indx + 1,
	...
	const subTotal = serviceTotal + medicineTotal + (examination.examinationFee ?? 0);

	let discountTotal = 0;
	const discounts = examination.discounts.map((discount, indx) => {
		const sum = discount.type === "fix"
			? discount.value
