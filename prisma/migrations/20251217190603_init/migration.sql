...
-- CreateEnum
CREATE TYPE "ExaminationStatus" AS ENUM (
  'BOOKED', 'WAITING', 'IN_PROGRESS', 
  'PENDING_PAYMENT', 'COMPLETED', 
  'CANCELLED'
);

-- CreateEnum
CREATE TYPE "ExaminationType" AS ENUM (
  'PRE_BOOKED', 'FOLLOW_UP', 'WALK_IN'
);
...
-- CreateFunction
CREATE 
OR REPLACE FUNCTION f_unaccent(text) RETURNS text AS $func$ 
SELECT 
  public.unaccent('public.unaccent', $1) $func$ LANGUAGE sql IMMUTABLE;

-- CreateFunction
CREATE 
OR REPLACE FUNCTION medicine_search_tsv_trigger() RETURNS trigger AS $$ BEGIN NEW."searchTsv" := setweight(
  to_tsvector(
    'pg_catalog.simple', 
    f_unaccent(NEW."name")
  ), 
...
-- CreateFunction
CREATE 
OR REPLACE FUNCTION article_search_vector_trigger() RETURNS trigger AS $$ BEGIN NEW."searchVector" := setweight(
  to_tsvector(
    'pg_catalog.simple', 
    f_unaccent(
      COALESCE(NEW."title", '')
...
-- CreateIndex
CREATE INDEX "Medicine_search_tsv_idx" ON "Medicine" USING GIN ("searchTsv");
...
-- CreateIndex
CREATE INDEX "Article_search_vector_idx" ON "Article" USING GIN ("searchVector");
...
-- CreateTrigger
CREATE TRIGGER medicine_search_tsv_update BEFORE INSERT 
OR 
UPDATE 
  ON "Medicine" FOR EACH ROW EXECUTE FUNCTION medicine_search_tsv_trigger();

-- CreateTrigger
CREATE TRIGGER article_search_vector_update BEFORE INSERT 
OR 
UPDATE 
  ON "Article" FOR EACH ROW EXECUTE FUNCTION article_search_vector_trigger();
